...
/*   Updated: 2020/03/13 17:30:42 by csouza-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	printpercent(t_mold *mold, va_list ap)
{
	...
	len += (len == 0) ? 0 : -1;
	mold->len += (mold->minus) ? putcs_x("%", ' ', len, 0) : 0;
	mold->len += (mold->zero) ? putcs_x("%", '0', len, 1) : 0;
	mold->len += (!mold->minus && !mold->zero) ? putcs_x("%", ' ', len, 1) : 0;
}
